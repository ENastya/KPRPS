...
public class TaskController implements Serializable {
    ...
    private TaskFacade getFacade() {
      ...
      public String stopProcessTask(models.Process cur){
        cur.calculateFullTime();
        cur.setEnded(true);
        getProcessFacade().edit(cur);
        getFacade().editStatus(cur.getTaskId(), 5);
        return preparePauseView(cur.getTaskId());
    }
    
    public String stopTask(Task task){
        getFacade().editStatus(task, 5);
        return "List";
    }
